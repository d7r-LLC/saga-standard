...
**Package**: `@d7r/saga-crypto` (new package in `saga-standard/packages/saga-crypto`)
...
**Package**: `@saga-standard/saga-client-rt` (new package — "rt" for runtime, distinct from existing `@d7r/saga-client` which is the HTTP API client)
...
| -------------------- | ------------------------------- | ------------------------------------------------ | ------------------------------------------------------------- |
| 1: Crypto Foundation | `@d7r/saga-crypto`              | KeyRing, envelope, encrypted store               | Unit: key derivation, encrypt/decrypt round-trips, store CRUD |
